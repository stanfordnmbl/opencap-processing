...
for k in tqdm(mcs_files):
    session_id = k
    
    trials = os.listdir(os.path.join(baseDir,'Data',session_id,'OpenSimData','Kinematics'))
    trials = [ x for x in trials if x.endswith('mot') and 'sqt' in x.lower() ]
    
    segments_file = os.path.join(baseDir, 'OpenCap-MCS-Segments','segments', session_id + '_SQT_1.npy')
    ...
    for i,segment in enumerate(segments_data['segments']):
        ...
        trial_name = f'SQT01_segment_{i+1}'
        print("Running on session:", session_id, "trial:",trial_name)
        
        orig_path = os.path.join(baseDir,'Data',session_id,'OpenSimData','Kinematics','SQT01.mot')
